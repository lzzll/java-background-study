 ...
 * 自定义认证类
 */
//@Component  // @Component和@Configuration都会初始化bean交给spring容器管理，@Configuration可以理解为是单例的，@Component是多例的
//@Configuration
//@Order(2)  // 不指定order的情况下和MultiHttpSecurityConfig中定义的内部类的配置的@Order(1)重复了
...
//    }
    ...
    /**
     * 这一步的配置是必不可少的，否则SpringBoot会自动配置一个AuthenticationManager,覆盖掉内存中的用户,导致内存中的用户没有角色
     */
    @Bean
    @Override
...
//            .access("hasAnyRole('ADMIN') and  hasRole('DBA')")
            ...
            .and()
            .formLogin()
//            .loginPage("/login_page")                      // 可自定义登陆页面，未初始化时采用springSecurity默认页面
